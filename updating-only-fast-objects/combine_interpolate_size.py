...
ds_factor = int(sys.argv[5]) / 100
window = float(sys.argv[6])
start_mag = float(sys.argv[7])
end_mag = float(sys.argv[8])
...
def main():
    ...
    min_mag_allowed = max_mag * start_mag
    max_mag_allowed = max_mag * end_mag 
    curr_mag = min_mag_allowed
    dirctn = 1 
    ...
    for frame_ct in tqdm(np.arange(0, total-1, frame_dist)):
        ...
        for i in range(mag.shape[0]):
            for j in range(mag.shape[1]):
                if mag[i][j] >= curr_mag and mag[i][j] < curr_mag + window:
        ...
        if curr_mag >= max_mag_allowed or curr_mag < min_mag_allowed:
            dirctn *= -1
        curr_mag = curr_mag + dirctn * max_mag * step_size
