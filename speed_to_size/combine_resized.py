...
def main():
	...
	(txt_width, txt_height), baseline = cv.getTextSize(filename, cv.FONT_HERSHEY_PLAIN, 1, 1)
	...
	for frame_ct in tqdm(np.arange(0, total-1, frame_dist)):
		...
		txt_img = cv.putText(pad_img, filename, (pad, pad + mag.shape[0] + txt_height+baseline), cv.FONT_HERSHEY_PLAIN, 1, (0,255,255), 1, cv.LINE_4)
